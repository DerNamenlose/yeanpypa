...
class InputReader(object):
    ...
    def getChar(self):
        ...
        if self.__ignore_white:
            self.skipWhite()
        logging.debug("Getting char at position %d: '%s'" % (self.__current_pos, self.__string[self.__current_pos]))
...
class ParseResult(object):
    ...
    def getRemaining(self):
        ...
        """
        return self.__input_reader.getInput()[self.__input_reader.getPos():]
...
class Rule(object):
    ...
    def __init__(self):
        self.__match = self.match
        self.match = self.__match_trace
    ...
    def match(self, input_reader):
        ...
        try:
            ...
            if char in self.__set:
                input_reader.rollback()
                raise ParseException("Expected char not from: [%s] at line %d:%d" % (self.__set, input_reader.line, input_reader.getPos()))
        except EndOfStringException:
            input_reader.rollback()
            raise ParseException("Expected char not from: [%s] at line %d:%d" % (self.__set, input_reader.line, input_reader.getPos()))
        input_reader.deleteCheckpoint()
    ...
    def match(self, input_reader):
        ...
        try:
            ...
            for rule in self.__subrules:
                ...
                if self.__autorule:
                    result = self.__autorule.match(input_reader)
                    if result != None:
                        retval.append(result)
            input_reader.deleteCheckpoint()
...
class Combine(Rule):
    ...
    def match(self, input_reader):
        ...
        logging.debug('Matched "%s"'%retval)
        return self.returnToken(self.callAction(retval))
...
class CallbackParser(Rule):
    ...
    def match(self, input_reader):
        ...
        if self.__callback(char):
            ...
            input_reader.rollback()
            raise ParseException('Character did not match %s at line %d' % (str(self), input_reader.line))
...
def parse(parser, string, ignore_white=True, trace=False):
    ...
    @type  ignore_white: boolean
    @param trace: Enable trace output of the rules (warning: slow!)
    @type trace: boolean
